...
(function () {
  ...
  /* ── Focus trap: keeps keyboard navigation inside the open menu ── */
  ...
  if (resForm && whatsappBtn) {
    ...
    whatsappBtn.addEventListener('click', () => {
      if (typeof resForm.requestSubmit === 'function') {
        resForm.requestSubmit();
      } else {
  ...
  /* ── Mobile sticky bar: show after hero, hide at reservations ── */
  const stickyBar    = document.getElementById('mobileStickyBar');
  const heroEl       = document.getElementById('hero');
  const resSectionEl = document.getElementById('reservations');
  ...
  if (stickyBar && heroEl) {
    ...
    // Hide when reservation form is visible
    if (resSectionEl) {
      new IntersectionObserver(([entry]) => {
        if (entry.isIntersecting) stickyBar.classList.remove('visible');
      }, { threshold: 0.2 }).observe(resSectionEl);
    }
  }
  ...
  document.addEventListener('click', e => {
    ...
    trackEvent('cta_click', { cta_label: label, page: window.location.pathname });
  });
  ...
  document.querySelectorAll('a[href^="tel:"]').forEach(el => {
    el.addEventListener('click', () =>
      trackEvent('phone_call', { page: window.location.pathname })
    );
  });
