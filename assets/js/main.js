...
(function () {
  ...
  if (audio && musicBtn) {
    ...
    function setPlaying(state) {
      ...
      musicLabel.textContent = state ? 'Now Playing' : 'Rema — Fun';
      musicBtn.setAttribute('aria-label', state ? 'Pause music' : 'Play music');
    }
    ...
    musicBtn.addEventListener('click', () => {
      if (playing) {
        ...
        audio.play()
          ...
          .catch(() => {
            /* Autoplay blocked — user gesture already happened via click, shouldn't reach here */
          });
      }
    });

    /* Pause automatically when hero scrolls out of view */
    const heroSection = document.getElementById('hero');
    if (heroSection) {
      const heroPauseObserver = new IntersectionObserver(
        ([entry]) => {
      ...
      heroPauseObserver.observe(heroSection);
    }

    /* Resume when user scrolls back to hero */
